...
class PointNetfeat(tf.keras.Model):
    ...
    def __init__(self, dimensions, dim_reduce_factor, args):
        ...
        if self.conv2lin:
            self.conv1 = Dense(F1, input_shape=(dimensions,), kernel_initializer=torch_default_lin_init, bias_initializer=torch_default_lin_init)
    ...
    def call(self, x, training=True):
        ...
        if training:
            x = tf.nn.relu(self.dr2(self.conv2(x)))
        else:
